...
static void usage(const char* prog) {
    std::cerr
        << "Usage:\n"
        << "  " << prog << " --method bs|mc --type call|put --spot S --strike K --T T --r r --sigma sigma [options]\n\n"
        << "Options:\n"
        << "  --method   bs|mc\n"
        << "  --type     call|put\n"
        << "  --mode     plain|anti|cv|anti+cv        (mc only)\n"
        << "  --paths    N                            (mc only, default 200000)\n"
        << "  --seed     uint64                       (mc only, default 123456)\n"
...
static void validate(const Args& a) {
    ...
    if (!std::isfinite(a.S) || !std::isfinite(a.K) || !std::isfinite(a.T) ||
        !std::isfinite(a.r) || !std::isfinite(a.sigma)) {
        bad("Missing required inputs. Provide --spot --strike --T --r --sigma");
    }
    ...
    if (a.method == "mc") {
        ...
        // mode string validated during parsing
    }
...
static Args parse_args(int argc, char** argv) {
    Args a;
    // Parse as: --key value, with --greeks as a flag
...
int main(int argc, char** argv) {
    try {
        ...
        if (a.method == "bs") {
            double price = NAN;
            if (a.type == "call") price = callPrice(a.S, a.K, a.T, a.r, a.sigma);
            else                  price = putPrice (a.S, a.K, a.T, a.r, a.sigma);
            ...
            if (a.greeks) {
                std::cout << "\nGreeks (Analytical)\n";
                if (a.type == "call") {
                    std::cout << "  delta:  " << std::setprecision(8) << callDelta(a.S,a.K,a.T,a.r,a.sigma) << "\n";
                } else {
                    std::cout << "  delta:  " << std::setprecision(8) << putDelta(a.S,a.K,a.T,a.r,a.sigma) << "\n";
                }
                std::cout << "  gamma:  " << std::setprecision(10) << gamma(a.S,a.K,a.T,a.r,a.sigma) << "\n";
                std::cout << "  vega:   " << std::setprecision(8) << vega(a.S,a.K,a.T,a.r,a.sigma) << "\n";
            }
